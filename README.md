...
temporary state below `/tmp` (independent of inherited `TEMP`/`TMP`), installs
the committed exact-hash lock, overlays the candidate
with `--no-index --no-deps --force-reinstall`, and runs `pip check`, full pytest,
the demo, and both snapshot consumers in a temporary Toolkit copy. During this
candidate run, the evidence freshness regression proves candidate-run byte
stability instead of requiring bytes generated by the still-pinned public
version. The ordinary CI run retains the committed-evidence equality check. The
helper discards subprocess output on failure and emits only a fixed stage
diagnostic.
